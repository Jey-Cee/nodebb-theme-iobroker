				...
				if (ajaxify.data.template['account/settings'] && !document.getElementById('persona:menus:legacy-layout')) {
					require(['translator'], function (translator) {
						...
						translator.translate('[[persona:show-signatures]]', function(translated){
							$('<div class="well checkbox"><label><input type="checkbox" id="persona:posts:signatures"/><strong>' + translated + '</strong></label></div>')
								.appendTo('#content .account > .row > div:first-child')
								.find('input')
								.prop('checked', Storage.getItem('persona:posts:signatures', 'true'))
								.change(function (e) {
									if (e.target.checked) {
										Storage.setItem('persona:posts:signatures', 'true');
									} else {
										Storage.removeItem('persona:posts:signatures');
									}
								});
						});
					});
